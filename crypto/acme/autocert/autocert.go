...
import (
	...
	"github.com/unix-world/smartgoext/crypto/acme"
	"github.com/unix-world/smartgoext/net/idna"
)
...
func supportsECDSA(hello *tls.ClientHelloInfo) bool {
	...
	schemeLoop:
		...
		//	const tlsECDSAWithSHA1 tls.SignatureScheme = 0x0203 // constant added in Go 1.10 // unixman
			switch scheme {
...
//			if curve == tls.CurveP256 {
			if curve == tls.CurveP384 { // unixman
				ecdsaOK = true
				break
			}
		}
...
//	data, err := m.Cache.Get(ctx, ck.String())
	data, err := m.Cache.Get(ctx, ck.String() + ".pem") // unixman
	if err != nil {
		return nil, err
	}
...
//	return m.Cache.Put(ctx, ck.String(), buf.Bytes())
	return m.Cache.Put(ctx, ck.String() + ".pem", buf.Bytes()) // unixman
}
...
func (m *Manager) certState(ck certKey) (*certState, bool, error) {
	...
	//	key, err = ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
		key, err = ecdsa.GenerateKey(elliptic.P384(), rand.Reader) // unixman
	}
	if err != nil {
...
func (m *Manager) deleteCertToken(name string) {
	...
	//	m.Cache.Delete(context.Background(), ck.String())
		m.Cache.Delete(context.Background(), ck.String() + ".pem") // unixman
	}
}
...
func (m *Manager) accountKey(ctx context.Context) (crypto.Signer, error) {
	...
	//	return ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
		return ecdsa.GenerateKey(elliptic.P384(), rand.Reader) // unixman
	}
