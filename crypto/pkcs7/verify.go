...
// UnmarshalSignedAttribute decodes a single attribute from the signer info, AuthenticatedAttributes
...
//-- unixman
// UnmarshalUnsignedAttribute decodes a single attribute from the signer info, UnauthenticatedAttributes
func (p7 *PKCS7) UnmarshalUnsignedAttribute(attributeType asn1.ObjectIdentifier, out interface{}) error {
	sd, ok := p7.raw.(signedData)
	if !ok {
		return errors.New("pkcs7: payload is not unsignedData content")
	}
	if len(sd.SignerInfos) < 1 {
		return errors.New("pkcs7: payload has no signers")
	}
