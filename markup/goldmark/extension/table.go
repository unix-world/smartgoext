...
//-- unixman
func parseCellAttributes(block text.Reader) (map[string]interface{}, bool) {
	var atts map[string]interface{} = map[string]interface{}{}
	attributes, ok := parser.ParseAttributes(block)
	if(!ok) {
		return atts, false
	}
	if(len(attributes) <= 0) {
	...
	for _, attr := range attributes {
		atts[string(attr.Name)] = attr.Value
	}
	return atts, true
}
//-- #
...
func (r *TableHTMLRenderer) renderTableCell(
	...
	if entering {
		...
		//-- unixman: ok
		if parser.WithAttribute() != nil {
		//	ns := node.Text(source)
			ns := n.Text(source)
			rdr := text.NewReader(ns)
			atts, ok := parseCellAttributes(rdr)
			if(!!ok && (len(atts) > 0)) {
				for key, val := range atts {
				//	n.SetAttribute([]byte(key), val)
					n.SetAttributeString(key, val)
				}
			}
		}
		//-- #
