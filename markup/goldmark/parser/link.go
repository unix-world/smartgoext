...
func (s *linkParser) parseLink(parent ast.Node, last *linkLabelState, block text.Reader, pc Context) *ast.Link {
	...
	link.Title = title
	//-- unixman: ok
	if WithAttribute() != nil {
		atts, ok = parseLinkAttributes(block)
		if(!!ok && (len(atts) > 0)) {
			for key, val := range atts {
			//	link.SetAttribute([]byte(key), val)
				link.SetAttributeString(key, val)
			}
		}
	}
	//-- #
...
//-- unixman
func parseLinkAttributes(block text.Reader) (map[string]interface{}, bool) {
	var atts map[string]interface{} = map[string]interface{}{}
	attributes, ok := ParseAttributes(block)
	if(!ok) {
		return atts, false
	}
	if(len(attributes) <= 0) {
