...
import (
	...
	"github.com/unix-world/smartgoext/markup/goldmark/ast"
...
//		c.XHTML = value.(bool)
	case optUnsafe:
		c.Unsafe = value.(bool)
	case optEscapedHtml: // unixman
		c.EscapedHtml = value.(bool)
	case optTextWriter:
...
//-- unixman
// Unsafe is an option name used in WithEscapedHtml.
const optEscapedHtml renderer.OptionName = "EscapedHtml"

type withEscapedHtml struct {
}

func (o *withEscapedHtml) SetConfig(c *renderer.Config) {
	c.Options[optEscapedHtml] = true
}

func (o *withEscapedHtml) SetHTMLOption(c *Config) {
	c.EscapedHtml = true
}

// WithEscapedHtml is a functional option that renders dangerous contents
// (raw htmls and potentially dangerous links) as it is.
func WithEscapedHtml() interface {
	renderer.Option
	Option
} {
	return &withEscapedHtml{}
}
//--
...
func (r *Renderer) renderHTMLBlock(
	...
	if entering {
		if r.EscapedHtml { // fix by unixman
			l := n.Lines().Len()
			for i := 0; i < l; i++ {
				line := n.Lines().At(i)
				r.Writer.SecureWrite(w, util.Nl2Br(util.EscapeHTML(line.Value(source))))
			}
		} else if r.Unsafe {
			l := n.Lines().Len()
			for i := 0; i < l; i++ {
		...
		//	_, _ = w.WriteString("<!-- raw HTML omitted -->\n")
			_, _ = w.WriteString("<!-- raw HTML omitted b:s -->\n") // fix by unixman
		}
	} else {
		if n.HasClosure() {
			if r.EscapedHtml { // fix by unixman
				closure := n.ClosureLine
				r.Writer.SecureWrite(w, util.Nl2Br(util.EscapeHTML(closure.Value(source))))
			} else if r.Unsafe {
				closure := n.ClosureLine
				r.Writer.SecureWrite(w, closure.Value(source))
			} else {
			//	_, _ = w.WriteString("<!-- raw HTML omitted -->\n")
				_, _ = w.WriteString("<!-- raw HTML omitted b:e -->\n") // fix by unixman
			}
		}
	}
	return ast.WalkContinue, nil
...
func (r *Renderer) renderParagraph(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	if entering {
		if n.Attributes() != nil {
		//	_, _ = w.WriteString("<p")
			_, _ = w.WriteString("<span") // unixman
	...
	//	_, _ = w.WriteString("</p>\n")
		_, _ = w.WriteString("</span><br>\n") // add <br> because paragraph is rendered as ablock and span is not
	}
	return ast.WalkContinue, nil
}
...
func (r *Renderer) renderText(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	n := node.(*ast.Text)
	segment := n.Segment
	value := segment.Value(source) // unixman
	// unixman:
	// clear attributes syntax on table cells can do only here, in renderTableCell there is no direct access to the cell content
	// syntax with attributes in table cells should be as `{attribute} cell content` (prepend, not append)
	// for unknown reasons using append of attributes into a table cell content as `cell content {attribute}` will in some cases split {attribute} on many lines and this fix will not work ...
	value = util.ClearAttributesSyntax(value) // unixman
	if n.IsRaw() {
	//	r.Writer.RawWrite(w, segment.Value(source)) // unixman
		r.Writer.RawWrite(w, value) // unixman
	} else {
	//	value := segment.Value(source) // unixman
...
func RenderAttributes(w util.BufWriter, node ast.Node, filter util.BytesFilter) {
	for _, attr := range node.Attributes() {
		...
		switch typed := attr.Value.(type) {
			...
			//-- unixman
			case float64:
				val := util.ConvertFloat64ToStr(typed)
				value = []byte(val)
				break
			case float32:
				val := util.ConvertFloat64ToStr(float64(typed))
				value = []byte(val)
				break
			case int64:
				val := util.ConvertInt64ToStr(typed)
				value = []byte(val)
				break
			case int32:
				val := util.ConvertInt64ToStr(int64(typed))
				value = []byte(val)
				break
			case int16:
				val := util.ConvertInt64ToStr(int64(typed))
			...
			case int:
				...
				break
			case uint64:
				val := util.ConvertUInt64ToStr(typed)
				value = []byte(val)
				break
			case uint32:
				val := util.ConvertUInt64ToStr(uint64(typed))
				value = []byte(val)
				break
			case uint16:
				val := util.ConvertUInt64ToStr(uint64(typed))
			...
			case uint:
				...
				break
			case bool:
				val := util.ConvertBoolsToStr(typed)
				value = []byte(val)
				break
			//-- #
		}
