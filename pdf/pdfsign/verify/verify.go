...
// modified by unixman:
// some signatures (ex: TSA) may fail, but not the last one
// because of this limitation, the last one must be non-tsa!
...
func Reader(file io.ReaderAt, size int64) (apiResp *Response, err error) {
	...
	for _, x := range rdr.Xref() {
		...
		//p7.Content = nil // possible fix for TSA signature verification ; untested ! ; by unixman
	...
	} else if(vfyLastErr != nil) {
		err = fmt.Errorf("Document was Modified, Last signature FAILED. Last Error Message: %s", vfyLastErr, " ; ERR=", vfyNumErr, " ; OK=", vfyNumOk)
	} else if((vfyNumErr > 0) && (vfyNumOk <= 0)) {
		err = fmt.Errorf("Failed to verify all signatures. Last Error Message: %s", vfyLastErr, " ; ERR=", vfyNumErr, " ; OK=", vfyNumOk)
	} else if(vfyNumOk <= 0) {
		err = fmt.Errorf("Failed to verify any signature. Last Error Message: %s", vfyLastErr, " ; ERR=", vfyNumErr, " ; OK=", vfyNumOk)
	}
	...
	fmt.Println("-------- Found Signatures:", vfyAll, "; FAIL:", vfyNumErr, "; OK:", vfyNumOk, "--------")
